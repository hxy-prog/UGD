...
import sys
import sys
sys.path.append('./BoT-SORT-main')
sys.path.append('.')
sys.path.append('./BoT-SORT-main/yolov7')
from tools.mc_demo_yolov7 import y7main
...
import time
import tempfile
from inference_backend import run_iml_inference, run_ciml_inference
...
import numpy as np
np.float = float  # 在 import torch 之前执行
np.int = int
# ... 然后才是你原来的代码
__appname__ = 'Multimedia Forensics System'
class export(QWidget):
    def __init__(self,filename,classname,savepath,mainwindow):
        super().__init__()
        self.format='json'
        ...
        self.cb.addItem("json")
        self.cb.addItem("xml")
        self.cb.addItem("txt")
        self.cb.currentIndexChanged.connect(self.getchange)
        layout2.addWidget(self.cb)
        self.btn1 = QPushButton('取消', self)  # 按下触发
        self.btn2 = QPushButton('确认', self)
        self.btn1.clicked.connect(self.cancel)
        self.btn2.clicked.connect(lambda :self.goon(filename,classname,savepath,mainwindow))
    ...
    def getchange(self):
        self.format=self.cb.currentText()
    def cancel(self):
        self.close()
    def goon(self,filename,classname,savepath,mainwindow):
        self.close()
        str = ("BoT-SORT-main/pretrained/yolov7-d6.pt", filename, classname, savepath,self.format)
        try:
            y7main(str, mainwindow)
        except Exception as e:
            print('\n')
            print(e)
            print('\n')
            exit(0)
def steptomodel(filename,classname,savepath,mainwindow):
    ex=export(filename,classname,savepath,mainwindow)
    ex.show()
class ComboxDemo(QWidget):
    def __init__(self,filename,savepath,mainwindow):
        super().__init__()
        # 设置标题
        self.setWindowTitle('选择标注类')
        self.clas='all'
        # 设置初始界面大小
        self.resize(400, 200)
        layout1=QVBoxLayout()
        ...
        self.btn2=QPushButton('确认',self)
        self.btn1.clicked.connect(self.cancel)
        self.btn2.clicked.connect(lambda :self.goon(filename,savepath,mainwindow))
        # 单个添加条目
        ...
        self.setLayout(layout1)
    def getchange(self):
        self.clas=self.cb.currentText()
    def goon(self,filename,savepath,mainwindow):
        self.close()
        steptomodel(filename,self.clas,savepath,mainwindow)
    def cancel(self):
        self.close()
    def cl(self,i):
        self.clas=i
...
class ResultViewer(QDialog):
    def __init__(self, title="检测结果"):
        ...
        self.labels = {}
        for name in ["original", "result", "heatmap"]:
            vbox = QVBoxLayout()
            label_title = QLabel(name.capitalize())
            label_title.setAlignment(Qt.AlignCenter)
            label_img = QLabel()
            label_img.setAlignment(Qt.AlignCenter)
            label_img.setMinimumSize(350, 350)
            label_img.setStyleSheet("border: 1px solid #ccc;")
            vbox.addWidget(label_title)
            vbox.addWidget(label_img)
            self.labels[name] = label_img
            layout.addLayout(vbox)

        self.setLayout(layout)

    def show_images(self, orig_path, result_path, heatmap_path):
        for key, path in zip(["original", "result", "heatmap"], [orig_path, result_path, heatmap_path]):
            if path and os.path.exists(path):
                pixmap = QPixmap(path)
                if not pixmap.isNull():
                    self.labels[key].setPixmap(
                        pixmap.scaled(
                            350, 350,
                            Qt.KeepAspectRatio,
                    ...
                    self.labels[key].setText("加载失败")
            else:
                self.labels[key].setText("无数据")
...
def concatenate_images(images, direction='vertical'):
    ...
    :param images: 图片路径列表
    :param direction: 拼接方向，'vertical' 或 'horizontal'
    :return: 合成后的图片对象
    """
...
def add_label_outside(image_path, label_text, position="top", margin=10):
    """
    在图像外侧（如顶部或底部）添加文本标签。
    
    :param image_path: 原始图像路径
    :param label_text: 要添加的文本
    :param position: 文本位置，支持"top"和"bottom"
    :param margin: 文本与图像间的间距
    """
    # 打开原始图像
    with Image.open(image_path) as img:
        ...
        # 根据需要扩展的新高度计算最终图像尺寸
        new_height = height + text_height + 2*margin
        
        # 创建一个带有白色背景的新图像（可以根据需要更改颜色）
        new_img = Image.new("RGB", (width, new_height), "white")
        
        # 将原始图像粘贴到新图像中，根据文本位置调整粘贴位置
        if position == "top":
            new_img.paste(img, (0, text_height + margin))
        else:  # 默认放置于顶部
            new_img.paste(img, (0, 0))
        
        # 重新创建绘图对象，这次是针对新创建的图像
        draw = ImageDraw.Draw(new_img)
        
        # 计算文本位置
        text_position = {
            "bottom": ((width - text_width) // 2, height + margin),
            "top": ((width - text_width) // 2, margin)
        }
        
        # 添加文本
        draw.text(text_position[position], label_text, fill="black", font=font)
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.canvas = Canvas(self,parent=self)
        ...
        open = action("打开图片", self.open_file,
                      'Ctrl+O', 'open', get_str('openFileDetail'))

        open_dir = action("打开图片目录", self.open_dir_dialog,
        ...
        def get_format_meta(format):
            ...
            if format == LabelFileFormat.PASCAL_VOC:
                return '&xml', 'format_voc'
            elif format == LabelFileFormat.YOLO:
                return '&txt', 'format_yolo'
            elif format == LabelFileFormat.CREATE_ML:
                return '&json', 'format_createml'
        ...
        self.atuosavepath=''
        choose_videofile=action('选择视频',self.choosefile,None,'file','选择视频')
        # choose_model=action('选择模型',self.choosefile,None,'file','选择模型')
        choose_thresh=action('选择阈值',self.choosethresh,None,'edit','选择阈值')
        choose_save_dir=action('选择保存目录',self.save_video_dir,None,'file','选择保存目录')
        newhelp=action('帮助',self.newhelp,None,'help','帮助')
        ...
        self.menus = Struct(
            ...
            atuolabel=self.menu('自动标注(A)'),
            recentFiles=QMenu(get_str('menu_openRecent')),
            labelList=label_menu)
        
                # === 新增：篡改检测菜单 ===
        self.menus.tamper = self.menu('篡改检测(T)')
        iml_action = QAction("单图检测 (IML)", self)
        iml_action.triggered.connect(self.run_iml_detection)
        ciml_action = QAction("双图一致性检测 (CIML)", self)
        ciml_action.triggered.connect(self.run_ciml_detection)
        add_actions(self.menus.tamper, (iml_action, ciml_action))
        ...
        add_actions(self.menus.atuolabel,(choose_videofile,choose_save_dir,choose_thresh,newhelp))
    ...
    def choosethresh(self):
        thre, okPressed = QInputDialog.getDouble(self, "阈值", "default:0.9", value=0.9, min=0, max=1,decimals=2)
        if okPressed:
            print(thre)
        self.atuolabelthresh=thre
    def newhelp(self):
        msgbox=QMessageBox.information(self,"说明","默认标注存放路径为./run/detect",QMessageBox.Ok,QMessageBox.Ok)
    def save_video_dir(self):
        path = os.path.dirname(ustr(self.file_path)) if self.file_path else '.'
        self.atuosavepath= QFileDialog.getExistingDirectory(self,"选取保存路径",path)
    def chooseclass(self,filename):
        self.comboxDemo = ComboxDemo(filename,self.atuosavepath,self)
        self.comboxDemo.show()
    def choosefile(self):
        if not self.may_continue():
            return
        path = os.path.dirname(ustr(self.file_path)) if self.file_path else '.'
        formats = ['*.%s' % fmt.data().decode("ascii").lower() for fmt in QImageReader.supportedImageFormats()]
        filters =  "视频文件(*.wmv *.avi *.mp4 *.mov);;JPEG Files(*.jpg)"
        filename, _ = QFileDialog.getOpenFileName(self, "选择视频文件", path, filters)
        if filename!='':
            self.chooseclass(filename)
    ...
    def run_iml_detection(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择待检测图像", "", "Images (*.png *.jpg *.jpeg *.bmp)"
        )
        if not file_path:
            return
        try:
            temp_dir = tempfile.mkdtemp(prefix="tamper_result_")
            unique_id = str(int(time.time()))
            QMessageBox.information(self, "提示", "正在执行 IML 推理，请稍候...", QMessageBox.Ok)
            QApplication.processEvents()
            
            result_path, heatmap_path = run_iml_inference(file_path, temp_dir, unique_id)

            # --- 合成三图（带标签）---
            original = add_label_outside(file_path, "Original Image")
            result = add_label_outside(result_path, "Result mask")
            heatmap = add_label_outside(heatmap_path, "Heatmap")
            combined_image = concatenate_images([original, result, heatmap], 'horizontal')
            combined_image_path = os.path.join(temp_dir, "combined_iml.jpg")
            combined_image.save(combined_image_path)

            # --- 关键：像打开普通图片一样加载合成图 ---
            self.load_file(combined_image_path)  # ← 使用已有的 load_file 方法！

            self.statusBar().showMessage("IML 推理完成。合成图已显示。", 5000)
            
        except Exception as e:
            import traceback
            traceback.print_exc()
            QMessageBox.critical(self, "错误", f"IML 推理失败:\n{str(e)}")


    def run_ciml_detection(self):
        files, _ = QFileDialog.getOpenFileNames(
            self, "选择两张图像（第一张参考，第二张待检）", "", 
            "Images (*.png *.jpg *.jpeg *.bmp)",
            options=QFileDialog.DontUseNativeDialog
        )
        if len(files) != 2:
            QMessageBox.warning(self, "提示", "请恰好选择两张图像！")
            return
        img1, img2 = files
        try:
            temp_dir = tempfile.mkdtemp(prefix="tamper_result_")
            unique_id = str(int(time.time()))
            QMessageBox.information(self, "提示", "正在执行 CIML 推理，请稍候...", QMessageBox.Ok)
            QApplication.processEvents()
            
            mask_path = run_ciml_inference(img1, img2, temp_dir, unique_id)

            # --- 合成三图（带标签）---
            ref_img = add_label_outside(img1, "Reference Image")
            test_img = add_label_outside(img2, "tampered Image")
            mask = add_label_outside(mask_path, "CIML Mask")
            combined_image = concatenate_images([ref_img, test_img, mask], 'horizontal')
            combined_image_path = os.path.join(temp_dir, "combined_ciml.jpg")
            combined_image.save(combined_image_path)

            # --- 关键：加载合成图 ---
            self.load_file(combined_image_path)  # ← 复用现有功能！

            self.statusBar().showMessage("CIML 推理完成。合成图已显示。", 5000)
            
        except Exception as e:
            import traceback
            traceback.print_exc()
            QMessageBox.critical(self, "错误", f"CIML 推理失败:\n{str(e)}")
    ...
    def show_bounding_box_from_annotation_file(self, file_path):
        ...
        if self.default_save_dir is not None:
            ...
            xml_path = os.path.join(filepath, basename + XML_EXT)
            txt_path = os.path.join(filepath, basename + TXT_EXT)
            json_path = os.path.join(filepath, basename + JSON_EXT)
