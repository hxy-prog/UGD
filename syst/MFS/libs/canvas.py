...
class parse():
    def __init__(self):
        self.weights = "BoT-SORT-main/pretrained/yolov7-d6.pt"
...
class Canvas(QWidget):
    ...
    def mouseDoubleClickEvent(self, ev):
        ...
        ###辅助快速标注
        opt=parse()
        path=self.parent().window().file_path
        if path!=None:
            if self.flag==1:
                self.flag=0
                opt.source=path
                self.result,self.labels=detect(opt)
                #xywh
                pos = self.transform_pos(ev.pos())
                for index,p in enumerate(self.result):
                    if  p[0]+p[2]/2>pos.x()>p[0]-p[2]/2 and p[1]+p[3]/2>pos.y()>p[1]-p[3]/2:
                        # ('bus', [(2526.0, 91.0), (3838.0, 91.0), (3838.0, 1788.0), (2526.0, 1788.0)], None, None, True)
                        shape=[]
                        for s in self.shapes:
                            point=(s.label,
                              [(s.points[0].x(), s.points[0].y()), (s.points[1].x(), s.points[1].y()),
                               (s.points[2].x(), s.points[2].y()), (s.points[3].x(), s.points[3].y())], None, None,
                              s.difficult)
                            shape.append(point)
                            self.main.remove_label(s)
                        point = (self.labels[index],
                                  [(p[0] - p[2] / 2, p[1] - p[3] / 2), (p[0] + p[2] / 2, p[1] - p[3] / 2),
                                   (p[0] + p[2] / 2, p[1] + p[3] / 2), (p[0] - p[2] / 2, p[1] + p[3] / 2)], None, None,
                                  False)
                        shape.append(point)
                        self.main.load_labels(shape)
            else:
                if self.result!=None:
                    pos = self.transform_pos(ev.pos())
                    for index,p in enumerate(self.result):
                        if p[0] + p[2] / 2 > pos.x() > p[0] - p[2] / 2 and p[1] + p[3] / 2 > pos.y() > p[1] - p[3] / 2:
                            ...
                            for s in self.shapes:
                                point = (s.label,
                                         [(s.points[0].x(), s.points[0].y()), (s.points[1].x(), s.points[1].y()),
                                          (s.points[2].x(), s.points[2].y()), (s.points[3].x(), s.points[3].y())], None,
                                         None,
                                         s.difficult)
                                shape.append(point)
                                self.main.remove_label(s)
                            point = (self.labels[index],
                                     [(p[0] - p[2] / 2, p[1] - p[3] / 2), (p[0] + p[2] / 2, p[1] - p[3] / 2),
                                      (p[0] + p[2] / 2, p[1] + p[3] / 2), (p[0] - p[2] / 2, p[1] + p[3] / 2)], None,
                                     None,
                                     False)
                            shape.append(point)
                            self.main.load_labels(shape)
