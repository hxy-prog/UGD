...
def make_parser():
    ...
    parser.add_argument("--fast-reid-weights", dest="fast_reid_weights", default=r"C:\Users\26387\Desktop\BoT-SORT-main\logs\MOT17\sbs_S50\model_0010.pth", type=str,help="reid config file path")
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    os.makedirs(save_folder, exist_ok=True)
    if args.demo == "video":
        save_path = osp.join(save_folder, "video.mp4")
    else:
        save_path = osp.join(save_folder, "camera.mp4")
    logger.info(f"video save_path is {save_path}")
    focc=cv2.VideoWriter_fourcc(*'mp4v')
    vid_writer = cv2.VideoWriter(
        save_path,focc, fps, (int(width), int(height))
    )
    tracker = BoTSORT(args, frame_rate=args.fps)
    timer = Timer()
    frame_id = 0
