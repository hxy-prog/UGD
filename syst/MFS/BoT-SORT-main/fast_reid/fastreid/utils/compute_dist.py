...
@author:  xingyu liao
@contact: sherlockliao01@gmail.com
@modified: Remove FAISS dependency, use sklearn instead.
"""
...
@torch.no_grad()
def compute_jaccard_distance(features, k1=20, k2=6, fp16=False):
    """
    Compute Jaccard distance using sklearn (no FAISS).
    """
    N = features.size(0)
    mat_type = np.float16 if fp16 else np.float32
    ...
    # Use sklearn to compute kNN (cosine or euclidean? Jaccard re-ranking usually uses Euclidean)
    # Note: FAISS used L2 (Euclidean), so we do the same.
    nbrs = NearestNeighbors(n_neighbors=k1 + 1, metric='euclidean', algorithm='auto').fit(feats_np)
    _, indices = nbrs.kneighbors(feats_np)  # shape: [N, k1+1], includes self at index 0

    # Remove self (index 0 is always the point itself)
    initial_rank = indices[:, 1:]  # shape: [N, k1]

    # But the original code expects [N, k1+1] including self at position 0
    # So we reconstruct it:
    self_indices = np.arange(N).reshape(-1, 1)
    initial_rank_full = np.concatenate([self_indices, initial_rank], axis=1)  # [N, k1+1]

    nn_k1 = []
    nn_k1_half = []
    for i in range(N):
        nn_k1.append(k_reciprocal_neigh(initial_rank_full, i, k1))
        nn_k1_half.append(k_reciprocal_neigh(initial_rank_full, i, int(np.around(k1 / 2))))
    ...
    if k2 != 1:
        V_qe = np.zeros_like(V, dtype=mat_type)
        for i in range(N):
            V_qe[i, :] = np.mean(V[initial_rank_full[i, :k2], :], axis=0)
        V = V_qe
