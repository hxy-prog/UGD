...
from ..trufor.net_utils import FeatureFusionModule as FFM
from ..trufor.net_utils import FeatureRectifyModule as FRM
import math
from ..trufor.early_conv import *
import time
...
class segformer(BaseModule):
    def __init__(self, pretrain_path=None, bonename='mit_b2',norm_layer=nn.BatchNorm2d):
        super(segformer, self).__init__()
        
        self.pretrain_path=pretrain_path
        # import backbone and decoder
        self.backbone = eval(bonename)(norm_fuse=norm_layer)
    ...
    def init_weights(self):
        if self.pretrain_path:
            ...
            filtered_state_dict = {key: value for key, value in checkpoint['state_dict'].items() if 'decode' not in key and 'detection' not in key}
            load_state_dict(self,filtered_state_dict,strict=True)
    ...
    def forward(self, rgb):
        if self.visit==0:
            self.dncnn.eval()
            self.visit+=1
        mean = torch.tensor([123.675, 116.28, 103.53], device=rgb.device).reshape(1, 3, 1, 1)
        std = torch.tensor([58.395, 57.12, 57.375], device=rgb.device).reshape(1, 3, 1, 1)
        # 进行 denormalize 操作
        denormalized_img = (rgb * std + mean)/255.0
        modal_x = self.dncnn(denormalized_img)

        modal_x = torch.tile(modal_x, (3, 1, 1))
        # rgb = preprc_imagenet_torch(rgb)
