...
class OutRefine(nn.Module):
    def __init__(self):
        super(OutRefine, self).__init__()
        # 第一个卷积层用于增加通道数
        self.conv1 = nn.Conv2d(2, 16, kernel_size=3, stride=1, padding=1)
        self.bn1 = nn.GroupNorm(4, 16)  # 使用 Group Normalization
        self.relu1 = nn.LeakyReLU(0.01, inplace=True)  # 使用 LeakyReLU
        
        # 第二个卷积层继续处理特征
        self.conv2 = nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1)
        self.bn2 = nn.GroupNorm(8, 32)  # 使用更多的组来精细化控制
        self.relu2 = nn.LeakyReLU(0.01, inplace=True)  # 继续使用 LeakyReLU
        
        # 第三个卷积层调整到目标通道数
        self.conv3 = nn.Conv2d(32, 4, kernel_size=1)  # 仅调整通道数，不改变空间尺寸
    ...
    def forward(self, x):
        ...
        x = self.pool(x)  # 调整到固定空间尺寸 32x32
        return x
@HEADS.register_module()
class DeformableHeadWithTime_sig(BaseDecodeHead):
    ...
    def forward(self, inputs, times):
        ...
        memory = memory.reshape(bs, cplus, h, w).contiguous()
        out = self.conv_seg(memory)
        out=self.refine(out)
        

        return out
    def forward_train_vae(self, inputs, times, img_metas, noisetrue, train_cfg):
        ...
        """
        noise = self(inputs, times)
        # print('www')
        # print(noise.size())
        # print(noisetrue.size())
        losses = dict()
