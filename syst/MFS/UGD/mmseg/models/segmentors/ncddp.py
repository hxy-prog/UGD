...
class nCDDP(CEncoderDecoder):
    ...
    def __init__(self,
                 ...
                 accumulation=False,
                 **kwargs):
        super(nCDDP, self).__init__(**kwargs)
    ...
    def encode_decode(self, img,img_metas,realimg):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        xreal= self.extract_feat(realimg)[0]
        ...
        if self.diffusion == "ddim":
            out = self.ddim_sample(x, xreal,img_metas)
    ...
    def forward_train(self, img, img_metas, realimg,gt_semantic_seg):
        ...
        noised_gt=torch.zeros((batch,self.decode_head.in_channels[0],h, w)).to(device)
        ...
        featreal = torch.cat([xreal, noised_gt], dim=1)
        featreal = self.transform(featreal)

        losses = dict()
        input_times = self.time_mlp(noise_level)
        loss_decode = self._decode_head_forward_train([feat,featreal], input_times, img_metas, gt_semantic_seg)
    ...
    def forward_test(self, imgs, img_metas, **kwargs):
        ...
        if num_augs == 1:
            return self.simple_test_logits(imgs, img_metas, **kwargs)[:,1,:,:]
        else:
            return self.aug_test(imgs, img_metas, **kwargs)
        
    @torch.no_grad()
    def ddim_sample(self, x, xreal,img_metas):
        ...
        for idx, (times_now, times_next) in enumerate(time_pairs):
            ...
            featreal = torch.cat([xreal, mask_t], dim=1)
            featreal = self.transform(featreal)
            ...
            mask_logit = self._decode_head_forward_test([feat,featreal], input_times, img_metas=img_metas)  # [bs, 150, ]
