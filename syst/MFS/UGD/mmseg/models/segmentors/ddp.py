...
class DDP(EncoderDecoder):
    ...
    def ddim_sample(self, x, img_metas):
        ...
        for idx, (times_now, times_next) in enumerate(time_pairs):
            ...
            import cv2
            # cv2.imwrite('ddimvis1/'+str(idx)+'.jpg',mask_pred[0].cpu().numpy()*256)
            mask_pred_cv2 = mask_pred[0].cpu().numpy()
            mask_pred_cv2 = mask_pred_cv2 * 256
            h, w = mask_pred_cv2.shape[:2]
            resized_mask = cv2.resize(mask_pred_cv2, (4*w, 4*h), interpolation=cv2.INTER_NEAREST)
            cv2.imwrite('ddimvis1/'+str(idx)+'.jpg', resized_mask)
