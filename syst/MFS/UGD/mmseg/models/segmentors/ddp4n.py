...
class NoiseRefine(nn.Module):
    def __init__(self, in_channels):
        super(NoiseRefine, self).__init__()
        self.conv1 = nn.Conv2d(
            in_channels=in_channels,  # 使用可配置的输入通道数
            out_channels=64,          # 第一层输出通道数
            kernel_size=3,            # 常用的3x3卷积核
            stride=1,
            padding=1                 # 填充1保持空间尺寸
        )
        self.relu1 = nn.ReLU()       # 添加非线性激活层
        self.conv2 = nn.Conv2d(
            in_channels=64,           # 第一层的输出作为第二层的输入
            out_channels=256,         # 最终输出通道数为256
            kernel_size=3,
            stride=1,
            padding=1
        )
...
class DDP4n(EncoderDecoder):
    ...
    def __init__(self,
                 ...
                 dimed=256,
                 **kwargs):
        super(DDP4n, self).__init__(**kwargs)
        ...
        self.embedding_table = nn.Embedding(self.num_classes + 1, dimed)
        self.noiserefine=NoiseRefine(dimed)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        noised_gt=self.noiserefine(noised_gt)
        # conditional input
    ...
    def ddim_sample(self, x, img_metas):
        ...
        mask_t = torch.randn((self.randsteps, self.dimed, h, w), device=device)
        outs = list()
        for idx, (times_now, times_next) in enumerate(time_pairs):
            re_mask_t=self.noiserefine(mask_t)
            feat = torch.cat([x, re_mask_t], dim=1)
            feat = self.transform(feat)
            log_snr = self.log_snr(times_now)
            log_snr_next = self.log_snr(times_next)

            padded_log_snr = self.right_pad_dims_to(re_mask_t, log_snr)
            padded_log_snr_next = self.right_pad_dims_to(re_mask_t, log_snr_next)
