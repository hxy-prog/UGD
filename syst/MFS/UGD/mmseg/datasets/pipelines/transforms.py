...
class Guassblur(object):
    ...
    def __call__(self,results):
        ...
        if isinstance(img, np.ndarray) and img.ndim == 3:
            ...
            raise TypeError("Image must be a numpy array with 3 dimensions.")
        return results
    def __repr__(self):
        return 'Guassblur'
    
@PIPELINES.register_module()
class Guassnoise(object):
    ...
    def __call__(self,results):
        ...
        # Clip the values to be in the proper range and convert back to uint8
        results['img'] = np.clip(noisy_image, 0, 255).astype(np.uint8)
        return results
    def __repr__(self):
        return 'Guassnoise' 
@PIPELINES.register_module()
class gamma_correction(object):
    ...
    def __call__(self,results):
        ...
        results['img']=cv2.LUT(img, table)
        return results
    def __repr__(self):
        return 'gamma_correction'

@PIPELINES.register_module()
class PairGuassblur(object):
    def __init__(self,kernel_size,sigma=0):
        self.kernel_size=kernel_size
        self.sigma=sigma
    def __call__(self,results):
        img = results['img']
        realimg=results['realimg']
        if isinstance(img, np.ndarray) and img.ndim == 3:
            results['img']=cv2.GaussianBlur(img, (self.kernel_size, self.kernel_size), self.sigma)
            results['realimg']=cv2.GaussianBlur(realimg, (self.kernel_size, self.kernel_size), self.sigma)
        else:
            raise TypeError("Image must be a numpy array with 3 dimensions.")
        return results
    def __repr__(self):
...
class PairGuassnoise(object):
    ...
    def __call__(self,results):
        ...
        # Add the Gaussian noise to the image
        noisy_image = img + gaussian
        noisy_realimage=realimg + gaussian
        # Clip the values to be in the proper range and convert back to uint8
        results['img'] = np.clip(noisy_image, 0, 255).astype(np.uint8)
        results['realimg'] = np.clip(noisy_realimage, 0, 255).astype(np.uint8)
        return results
    def __repr__(self):
        return 'Guassnoise' 
    
@PIPELINES.register_module()
class Pairgamma_correction(object):
    def __init__(self,gamma=0):
        self.gamma=gamma
    def __call__(self,results):
        img = results['img']
        ...
        for i in np.arange(256)]).astype("uint8")
        results['img']=cv2.LUT(img, table)
        results['realimg']=cv2.LUT(realimg, table)
        return results
    def __repr__(self):
        return 'gamma_correction'
...
class JPEGCompression(object):
    def __init__(self, quality_lower=30, quality_upper=100, p=0.5):
        """初始化Albumentations的ImageCompression变换"""
        self.p=p
    ...
    def __call__(self, results):
        ...
        if random.random()>self.p:
            return results
        img = results['img']
        if isinstance(img, np.ndarray) and img.ndim == 3:  # HWC格式
            # 应用Albumentations变换
...
@PIPELINES.register_module()
class PairJPEGCompression(JPEGCompression):  
    def __init__(self, quality_lower=30, quality_upper=100, p=0.5):
        """初始化Albumentations的ImageCompression变换"""
        self.p=p
        self.transform = A.Compose([
    ...
    def __call__(self, results):
        ...
        if isinstance(img, np.ndarray) and img.ndim == 3:  # HWC格式
            ...
            transformed = self.transform(image=img)
            results['img'] = transformed['image']
            transformed = self.transform(image=realimg)
            results['realimg'] = transformed['image']
        else:
            raise TypeError("Image must be a numpy array with 3 dimensions.")
        return results
...
class Resize(object):
    ...
    def __repr__(self):
        ...
        return repr_str
@PIPELINES.register_module()
class SizeResize(object):
    def __init__(self,size):
        self.size=size
    def __call__(self, results):
        img,_,_= mmcv.imresize(
            results['img'], self.size, return_scale=True)
        for key in results.get('seg_fields', []):
            gt_seg = mmcv.imresize(
                    results[key], self.size, interpolation='nearest')
            results[key] = gt_seg
        results['img'] = img
        results['img_shape'] = img.shape
        results['pad_shape'] = img.shape  # in case that there is no padding
    ...
    def __repr__(self):
        ...
        return repr_str
@PIPELINES.register_module()
class PairSizeResize(object):
    def __init__(self,size):
        self.size=size
    def __call__(self, results):
        img,_,_= mmcv.imresize(
            results['img'], self.size, return_scale=True)
        realimg,_,_= mmcv.imresize(
            results['realimg'], self.size, return_scale=True)
        for key in results.get('seg_fields', []):
            gt_seg = mmcv.imresize(
                    results[key], self.size, interpolation='nearest')
            results[key] = gt_seg
        results['img'] = img
        results['realimg'] = realimg
        results['img_shape'] = img.shape
        results['pad_shape'] = img.shape  # in case that there is no padding
        # results['scale_factor'] = scale_factor
        results['keep_ratio'] = False
...
class PairResize(Resize):
    def _resize_img(self, results):
        ...
        if self.keep_ratio:
            ...
            img, scale_factor = mmcv.imrescale(
                results['img'], results['scale'], return_scale=True)
            realimg, scale_factor = mmcv.imrescale(
                results['realimg'], results['scale'], return_scale=True)
            # the w_scale and h_scale has minor difference
            # a real fix should be done in the mmcv.imrescale in the future
            new_h, new_w = img.shape[:2]
            h, w = results['img'].shape[:2]
            ...
            img, w_scale, h_scale = mmcv.imresize(
                results['img'], results['scale'], return_scale=True)
            realimg, w_scale, h_scale = mmcv.imresize(
                results['realimg'], results['scale'], return_scale=True)
        scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                dtype=np.float32)
        results['img'] = img
...
class PairRandomFlip(RandomFlip):
    def __call__(self, results):
            ...
            if results['flip']:
                ...
                results['img'] = mmcv.imflip(
                    results['img'], direction=results['flip_direction'])
                results['realimg'] = mmcv.imflip(
                    results['realimg'], direction=results['flip_direction'])
...
class PairPad(Pad):
    def _pad_img(self, results):
            ...
            if self.size is not None:
                ...
                padded_realimg = mmcv.impad(
                    results['realimg'], shape=self.size, pad_val=self.pad_val)
            elif self.size_divisor is not None:
                padded_img = mmcv.impad_to_multiple(
                    results['img'], self.size_divisor, pad_val=self.pad_val)
                padded_realimg = mmcv.impad(
                    results['realimg'], shape=self.size, pad_val=self.pad_val)
            results['img'] = padded_img
            results['realimg'] = padded_realimg
            results['pad_shape'] = padded_img.shape
            results['pad_fixed_size'] = self.size
            results['pad_size_divisor'] = self.size_divisor
@PIPELINES.register_module()
...
class PairNormalize(Normalize):
    def __call__(self, results):
            ...
            results['img'] = mmcv.imnormalize(results['img'], self.mean, self.std,
                                            self.to_rgb)
            results['realimg'] = mmcv.imnormalize(results['realimg'], self.mean, self.std,
                                            self.to_rgb)
            results['img_norm_cfg'] = dict(
                mean=self.mean, std=self.std, to_rgb=self.to_rgb)
            return results
...
class PairRandomCrop(RandomCrop):
    def __call__(self, results):
        ...
        # crop the image
        img = self.crop(img, crop_bbox)
        realimg = self.crop(realimg, crop_bbox)
        img_shape = img.shape
        results['img'] = img
        results['realimg'] = realimg
        results['img_shape'] = img_shape
